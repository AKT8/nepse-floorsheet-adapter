...
function buildUrl(date, pageIndex, pageParam) {
...
async function getJson(url, attempt = 1) {
    ...
    try {
        ...
        if (!json.success)
            throw new Error(`API responded success=false: ${json.message ?? "no message"}`);
        return json;
    }
    catch (err) {
 ...
 */
async function detectPageParam(date) {
...
async function fetchPage(date, pageIndex) {
...
export async function fetchFullDayRaw(date) {
    const first = await fetchPage(date, 1);
    const seen = new Map();
    for (const t of first.content)
        seen.set(t.contractId, t);
    ...
    return Array.from(seen.values()).sort((a, b) => new Date(a.tradeTime).getTime() - new Date(b.tradeTime).getTime());
}
/** Fetches just page 1 (cheap peek - used by the live poller to check for new trades). */
export async function fetchLatestPageRaw(date) {
